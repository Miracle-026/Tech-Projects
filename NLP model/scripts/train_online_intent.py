"""
Training script for online intent classification model.
"""
...
from src.models.online_intent import OnlineIntentAnalyzer, IntentExample
from src.utils.data_loader import load_intent_data

def setup_logging(verbose: bool) -> logging.Logger:
    """Setup logging configuration."""
    logger = logging.getLogger("OnlineIntentTraining")
...
def main():
    parser = argparse.ArgumentParser(description="Train online intent classification model")
    parser.add_argument('--config', type=str, default='configs/online_config.yaml',
                        help='Path to configuration file')
    parser.add_argument('--data-dir', type=str, default='datasets/intent/',
                        help='Directory containing intent datasets')
    ...
    for task_id, domain in enumerate(domains):
        ...
        if train_file.exists():
            train_data = load_intent_data(str(train_file), task_id, domain)
            train_datasets[domain] = train_data
        if test_file.exists():
            test_data = load_intent_data(str(test_file), task_id, domain)
    ...
    for task_id, domain in enumerate(domains):
        ...
        # Save checkpoint
        checkpoint_path = model.save_checkpoint(f'online_intent_{domain}_task_{task_id}.pkl')
